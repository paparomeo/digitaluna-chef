...
CHEF_DATA_DIR=/var/chef
CHEF_INSTALL_SCRIPT=${CHEF_DATA_DIR}/install.sh
...
ssh -t $1 "sudo bash -c \"apt-get update && apt-get dist-upgrade && apt-get -y install git && rm -rf ${CHEF_DATA_DIR} && git clone ${CHEF_REPO_URL} ${CHEF_DATA_DIR} && ${CHEF_INSTALL_SCRIPT}\""